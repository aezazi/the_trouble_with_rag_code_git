...
# %%
# extract text chunks from the 'positive_wrong', 'positive', and 'negative' category in the en_fact.json dataset
...
with open('./data/en_fact.json', 'r') as f:
    for i, line in enumerate(f):
        ...
        # print(data_line)
        # print(type(data_line['negative'][0]))
        # print(len(data_line['negative']))
        # print(data_line.keys())
        # break
        
        # data_line['positive'] is a nested_list
        for text_chunk in data_line['positive']:
            # each element in data_line['positive'] is a text chunk
        ...
        # each element in data_line['positive_wrong'] is a text chunk
        # each text chunk will be vectorized and will be the text we will retrieve
        for text_chunk in data_line['positive_wrong']:
            metadata_dict = {}
            vector_id_list.append(str(vector_count))
            text_chunk_list.append(text_chunk)
            ...
            metadata_dict['query'] = data_line['query']
            metadata_dict['text'] = text_chunk
            metadata_dict['category'] = 'positive_wrong'
            metadata_list.append(metadata_dict)
            
            vector_count += 1  

        # each element in data_line['negative'] is a text chunk
        # each text chunk will be vectorized and will be the text we will retrieve
...
df_small.to_pickle('./df_en_fact_all_vecs_small.pkl')
df_large.to_pickle('./df_en_fact_all_vecs_large.pkl')
...
index_1 = 'en-fact-all-small'
index_2 = 'en-fact-all-large'
...
# %%
# insert vectors into Pinecone indexes
batch_size = 100
...
for i in range(0, row_count_small, batch_size):
    ...
    pine.Index('en-fact-all-small').upsert(vectors=zip(df_batch.id, 
...
for i in range(0, row_count_large, batch_size):
    ...
    pine.Index('en-fact-all-large').upsert(vectors=zip(df_batch.id, 
                                        df_batch.vectors, df_batch.metadata)) 

#%%
#get stats on the indexes

display(pine.Index('en-fact-all-small').describe_index_stats())
display(pine.Index('en-fact-all-large').describe_index_stats())
