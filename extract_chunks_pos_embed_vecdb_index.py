...
# %%
#embed text chuncks with small and large openAI models
embedded_text_chunks_small = get_embedding(text_chunk_list, 
                                           model='text-embedding-3-small')

embedded_text_chunks_large = get_embedding(text_chunk_list, 
...
embedded_text_chunks_small_vectors = [embedded_text_chunks_small[i].embedding for i in range(len(embedded_text_chunks_small)) ]
...
df_small.to_pickle('./df_en_int_positive_vecs_small.pkl')
df_large.to_pickle('./df_en_int_positive_vecs_large.pkl')
...
index_1 = 'en-int-positive-small'
index_2 = 'en-int-positive-large'
...
for i in range(0, row_count_small, batch_size):
    ...
    pine.Index('en-int-positive-small').upsert(vectors=zip(df_batch.id, 
...
for i in range(0, row_count_large, batch_size):
    ...
    pine.Index('en-int-positive-large').upsert(vectors=zip(df_batch.id, 
                                        df_batch.vectors, df_batch.metadata)) 

#%%
#get stats on the indexes

display(pine.Index('en-int-positive-small').describe_index_stats())
display(pine.Index('en-int-positive-large').describe_index_stats())
